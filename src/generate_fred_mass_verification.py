...
python generate_fred.py --start-date "2019-09-01 00:00" --end-date "2019-09-01 08:30"
...
def main():
    ...
    parser.add_argument('-a', '--alpha', dest='alpha', type=float, default=0.5, help='Alpha weight for polar orbiting satellites')
    ...
    parser.add_argument('-n', '--name', dest="prefix_name", default='SEP_0.7_12_2km_', help="Prefix for output file names")
    ...
    for days in range(0, 10):
        ...
        list_sunglint_angle = []
        for max_sunglint_angle in range(0, 24, 2):
           geo_df['geo_weight_' + str(max_sunglint_angle)] = geo_df['geo_weight']
           sun_glint_flag = (geo_df['sunglint_angle'] <= max_sunglint_angle) & (geo_df['satellite'] == "Himawari-8/9")
           geo_df.loc[sun_glint_flag, 'geo_weight_' + str(max_sunglint_angle)] = 0.0
           list_sunglint_angle.append('geo_weight_' + str(max_sunglint_angle))
        ...
        try:
            ...
            grid_weight_total = df_grid_joined[(['id', 'weight', 'geo_weight', 'polar_weight'] + list_sunglint_angle)]\
               .groupby(['id']).sum()
